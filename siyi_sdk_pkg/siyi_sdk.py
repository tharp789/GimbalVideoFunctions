...
class SIYISDK:
    ...
    def resetVars(self):
        ...
        self._request_gimbal_camera_codec_specs_ack_msg = GimbalCameraCodecSpecsMsg()
        self._send_gimbal_camera_codec_specs_msg = SendGimbalCameraCodecSpecsMsg()
        self._send_gimbal_camera_codec_specs_ack_msg = SendGimbalCameraCodecSpecsAckMsg()
        self._request_gimbal_camera_image_mode_msg = RequestGimbalCameraImageModeMsg()
        self._send_gimbal_camera_image_mode_msg = SendGimbalCameraImageModeMsg()
        self._send_thermal_palette_msg = SendThermalPaletteMsg()
        self._request_thermal_palette_msg = RequestThermalPaletteMsg()
        self._request_absolute_zoom_msg = RequestAbsoluteZoomMsg()
    ...
    def bufferCallback(self):
        ...
        while(len(buff_str)>=MINIMUM_DATA_LENGTH):
            ...
            elif cmd_id==COMMAND.SEND_GIMBAL_CAMERA_IMAGE_MODE:
            ...
            elif cmd_id==COMMAND.SEND_THERMAL_PALETTE:
                #print("-Send thermal palette")
                self.parseSendThermalPaletteMsg(data, seq)
            elif cmd_id==COMMAND.REQUEST_THERMAL_PALETTE:
                #print("-Request thermal palette")
                self.parseRequestThermalPaletteMsg(data, seq)
            else:
                self._logger.warning("Unknown command ID: %s", cmd_id)
        return
    ...
    def sendGimbalCameraCodecSpecs(self, stream_type: int, video_enc_type: int, resolution_l: int, resolution_h: int, video_bitrate: int):
        """
        Sends the gimbal camera codec specs

        Params
        ---
        stream_type: [int] 0: recording stream, 1: main stream, 2: sub stream
        video_enc_type: [int] 1: H.264, 2: H.265 / HVEC
        resolution_l: [int] 1920 or 1280 for RGB, 680 for Thermal, in px
        resolution_h: [int] 1080 or 720 for RGB, 512 for Thermal, in px
        video_bitrate: [int] 0-3000 for RGB, in kbps
        """
    ...
    def sendGimbalCameraImageMode(self, vdisp_mode: int):
        """
        Sends the gimbal which feed to put to main and sub streams. 
    ...
    def requestThermalPalette(self):
        """
        Sends request for thermal palette
        """
        msg = self._out_msg.requestThermalPaletteMsg()
        return self.sendMsg(msg)
    
    def sendThermalPalette(self, palette: int):
        """
        Sends the gimbal camera codec specs
        ---
        palette: [int] 0-7
        ...
        11: Glory_Hot 
        """
        msg = self._out_msg.sendThermalPaletteMsg(palette)
        return self.sendMsg(msg)
    ...
    def parseRequestGimbalCameraCodecSpecsMsg(self, msg:str, seq:int):
        ...
        try:
            ...
            self._request_gimbal_camera_codec_specs_ack_msg.seq = seq
            self._request_gimbal_camera_codec_specs_ack_msg.stream_type = stream_type
            self._request_gimbal_camera_codec_specs_ack_msg.video_enc_type = video_enc_type
            self._request_gimbal_camera_codec_specs_ack_msg.resolution_l = res_l
            self._request_gimbal_camera_codec_specs_ack_msg.resolution_h = res_h
            self._request_gimbal_camera_codec_specs_ack_msg.video_bitrate = bitrate
    ...
    def parseSendGimbalCameraCodecSpecsMsg(self, msg:str, seq:int):
        ...
        """
        try:
            self._send_gimbal_camera_codec_specs_ack_msg.seq = seq
            self._send_gimbal_camera_codec_specs_ack_msg.stream_type = int('0x'+msg[0:2], base=16)
            self._send_gimbal_camera_codec_specs_ack_msg.sta = bool(int('0x'+msg[2:4], base=16))
            return True
        except Exception as e:
            self._logger.error("Error parsing gimbal camera codec specs send: %s", e)
            self._send_gimbal_camera_codec_specs_ack_msg.sta = 0 # Failed
            return False
    ...
    def parseSendThermalPaletteMsg(self, msg:str, seq:int):
        """
        Parse the thermal palette message
        """

        thermal_palette = int(msg, 8)
        ...
        try:
            self._send_thermal_palette_msg.seq = seq
            self._send_thermal_palette_msg.palette = thermal_palette
            self._send_thermal_palette_msg.description = palette_mode_map.get(thermal_palette, "Unknown palette")
            return True
        except Exception as e:
            self._logger.error("Error %s", e)
            return False
        
    def parseRequestThermalPaletteMsg(self, msg:str, seq:int):
        """
        Parse the thermal palette message
        """
        ...
        try:
            self._request_thermal_palette_msg.seq = seq
            self._request_thermal_palette_msg.palette = thermal_palette
            self._request_thermal_palette_msg.description = palette_mode_map.get(thermal_palette, "Unknown palette")
            return True
        except Exception as e:
            self._logger.error("Error %s", e)
            return False
    ...
    def getGimbalCameraCodecSpecs(self):
        return(self._request_gimbal_camera_codec_specs_ack_msg.stream_type, self._request_gimbal_camera_codec_specs_ack_msg.video_enc_type, self._request_gimbal_camera_codec_specs_ack_msg.resolution_l, self._request_gimbal_camera_codec_specs_ack_msg.resolution_h, self._request_gimbal_camera_codec_specs_ack_msg.video_bitrate)
        
    def getSendGimbalCameraCodecSpecs(self):
        return(self._send_gimbal_camera_codec_specs_ack_msg.stream_type, self._send_gimbal_camera_codec_specs_ack_msg.sta)
    ...
    def getThermalPalette(self):
        return(self._request_thermal_palette_msg.palette, self._request_thermal_palette_msg.description)
